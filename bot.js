...
// ID-ul mesajului de reaction role (se setează automat cu !setuprol)
...
client.on('messageReactionAdd', async (reaction, user) => {
  if (user.bot) return;
  if (!reactionMessageId || reaction.message.id !== reactionMessageId) return;
...
client.on('messageCreate', async (message) => {
  ...
  // =====================
  // COMANDA !setuprol (doar admin)
  // =====================
  if (command === 'setuprol') {
    if (!message.member.permissions.has('Administrator')) {
      return message.reply('❌ Doar adminii pot folosi această comandă!');
    }

    const canal = message.guild.channels.cache.find(c => c.name === '🎭-badge-uri');
    if (!canal) {
      return message.reply('❌ Canalul 🎭-badge-uri nu a fost găsit!');
    }

    const embed = new EmbedBuilder()
      .setColor(0xf4a300)
      .setTitle('🚚 DEVINO CURIER OFICIAL! 🚚')
      .setDescription(
        `Vrei să faci parte din echipa noastră de elită?\n\n` +
        `Reacționează cu **🚚** la acest mesaj pentru a primi rolul **CURIER**!\n\n` +
        `*Retrage reacția dacă vrei să renunți la rol.*`
      )
      .setFooter({ text: 'Curierii — comunitatea oficială' })
      .setTimestamp();
    ...
    await message.reply(`✅ Mesajul de reaction role a fost trimis în <#${canal.id}>!`);
  }
